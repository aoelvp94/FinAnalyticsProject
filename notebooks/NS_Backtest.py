...
def load_and_join():
    ...
    df = pd.read_excel(
        "https://images.aqr.com/-/media/AQR/Documents/Insights/Data-Sets/Century-of-Factor-Premia-Monthly.xlsx",
        header=18,
        nrows=1220,
    )
...
def labeling_df(df):
    ...
    df.loc[
        df["50_days_average"] >= df["200_days_average"], "Buy/Sell"
    ] = -1  # Si la media de corto plazo supera a la de largo, es posicion de sell
    df.loc[
        df["50_days_average"] < df["200_days_average"], "Buy/Sell"
    ] = 1  # Si la media de corto plazo esta por debajo de la de largo, es posicion de buy
    return df
...
def back_test(df, mode, start_date, end_date):
    ...
    if mode == "Simple":
        ...
        trades = pd.DataFrame(
            columns=["Opening_Price", "Closing_Price", "Return", "Type"]
        )
        position = 0
        ...
        for i in range(len(trades)):
            ...
            print(
                "Abrio al precio de $ {:.2f} y cerro en $ {:.2f}".format(
                    trades.iloc[i, 0], trades.iloc[i, 1]
                )
            )
        ...
        trades = pd.DataFrame(
            columns=["Opening_Price", "Closing_Price", "Return", "Type", "Bet Size"]
        )
        position = 0
        for i in range(len(df1)):
            if position == 0:
        ...
        for i in range(len(trades)):
            print("Trade {}, tipo: {}".format(i, trades.iloc[i, 3]))
            print(
                "Abrio al precio de $ {:.2f} y cerro en $ {:.2f}, con un size de {:.2f}".format(
                    trades.iloc[i, 0], trades.iloc[i, 1], trades.iloc[i, 4]
                )
            )
            print("Retorno: {:.2%}".format(trades.iloc[i, 2]))
            print("*********************************************")
